...
* `SINGLE_MESSAGE`        единичные сообщения шифрованные CPubK адресата
    ...
    * `data(TB)`: зашифрованные CPrK данные, содержат список `COMMAND`
...
* `COMMAND_RESULT`  ответ на команду IPC <br/> подтверждение приема команды (с соотв. кодом), отправляется в том случае
    если команда не поддерживает ответ, или ответ не может быть сформирован немедленно
    * `id(T04)`: идентификатор команды на которую отправлется ответ
    * `code`: код результата (в зависимости от кода опредлеяется ошибка это, подтверждение приема или полноценный ответ)
    * `value`: результат

* `STR_HEAD` - заголовок потока и настройки блочного шифрования
     * `CIPHER_OPTIONS` **а где ключ?**
